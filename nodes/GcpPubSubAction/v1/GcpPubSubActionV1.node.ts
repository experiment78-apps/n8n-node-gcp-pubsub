	...
	description: INodeTypeDescription = {
		...
		properties: [
						...
						validation: [
							{
								...
								properties: {
									...
									errorMessage:
										'Enter a short subscription name (3-255 chars, cannot start with "goog"), a full resource path (projects/.../subscriptions/...), or an expression.',
								},
							},
						],
					},
			...
			},
			{
				...
				default: '={{$json.ackId}}',
				description:
					'The ackId of the message delivery to operate on. Defaults to the value emitted by the trigger.',
				required: true,
			},
			{
			...
			},
			{
				...
				default: '',
				description:
					'Google Cloud project ID. Only needed when the Subscription field is a short name and the credential does not include a project ID.',
			},
			{
				displayName: 'Options',
				name: 'options',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (!ackId) {
					throw new NodeOperationError(this.getNode(), 'ackId is required', {
						itemIndex: i,
					});
				}
